...
struct Node_s {
    Node * next;
    Node * prev;
    void * dataPtr; // Holds a pointer to the data or struct contained in the node
};
...
struct List_s {
    ...
    void * currItemPtr;
    int nodeCount;
    int position;//Position of the currNode iterator like behaviour -1 == beyond the tail 0 == before the head

    //GLOBAL STATIC ACCESS VARIABLES
    Node * nodeArr; //A pointer to pointer reperseting a statically allocated nodes avaliable for allocation
    int * nodeArrIdx; //Index of free node
    int * freedNodesCount;//The number of freeNoes
    Node ** freeNodesArr; //Pointer to a pointer representing the free nodes
    List ** freeListArr;
    int * freeListCount;
};
